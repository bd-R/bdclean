...
get_config <- function(quest = NULL) {
    if (is.null(quest)) {
        quest <-
            data.frame(
                ...
                question = c(
                    "What is the lowest taxonomic level you require in your data? ",
    ...
    options <-
        list(
            taxoLevel = data.frame(
                choice = c(1, 2, 3, 4),
                value = c("SUBSPECIES", "SPECIES", "GENUS", "FAMILY")
            ),
            misNames = data.frame(choice = c(1, 2), value = c("Yes", "No")),
            spatialResolution = NULL,
            dateCheck = data.frame(choice = c(1, 2), value = c("Yes", "No")),
            earliestDate = NULL,
            temporalResolution = data.frame(
                choice = c(1, 2, 3),
                value = c("Year", "Month", "Day")
            )
        )
